...
import { handleJellyfinJobPost } from '../services/jellyfinMethods';
...
export async function handlePostRequest(
  ...
  if (requestUrl.searchParams.has('upload')) {
    ...
    if (
      ...
      const needGlobal = isGlobalGateActive(env) && !verifyGlobalPassword(env, body.globalPasswd);
      return new Response(JSON.stringify({ error: 'access denied', needGlobal }), {
        status: 403,
  ...
  const filesRes = isListAllowed
    ...
    : {
        ...
        files: [],
        encrypted: true,
        needGlobal: isGlobalGateActive(env) && !verifyGlobalPassword(env, body.globalPasswd),
      };
